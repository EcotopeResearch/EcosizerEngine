...
def mixing_valve_behavior(load_supplyT_gal : float, flow_returnT_gal : float, cold_temp_f : float, supply_temp_f : float, return_temp_f : float, storage_temp_f : float) -> dict:
    ...
    critical_flow_g = recirc_loss_btu / (_RHO_CP * (storage_temp_f - supply_temp_f))

    if load_supplyT_gal > critical_flow_g:
        ...
        storage_draw_gal = load_supplyT_gal * ((supply_temp_f - return_temp_f) / (storage_temp_f - return_temp_f))
