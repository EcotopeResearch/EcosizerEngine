...
class SwingTank(SystemConfig):
    ...
    def __runOneSwingStep(self, building, swingheating, Tcurr, hw_out):
        ...
        if swingheating:
            ...
            if Tnew <= building.supplyT_F: # If the element should turn on
    ...
    def simulate(self, building, initPV=None, initST=None, Pcapacity=None, Pvolume=None):
        ...
        ------
        building : Building
            The building the system is being simulated for
        initPV : float
        ...
        for i in range(1, len(G_hw)):
            ...
            swingheating, swingT[i], srun[i] = self.__runOneSwingStep(building, swingheating, swingT[i-1], hw_outSwing[i])
            #Get the mixed generation
            mixedGHW = mixVolume(G_hw[i], mixedStorT_F, building.incomingT_F, building.supplyT_F) #replaced self.storageT_F with mixedStorT_F
    ...
    def plotStorageLoadSim(self, building, return_as_div=True):
        """
        ...
        [V, G_hw, D_hw, run, swingT, srun, _] = self.simulate(building)
        ...
        fig.update_yaxes(title_text="Swing Tank\nTemperature (\N{DEGREE SIGN}F)",
                            ...
                            secondary_y=False, range=[building.supplyT_F-5, self.storageT_F])
