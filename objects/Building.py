...
class Building(ABC):
    def __init__(self, inputs):
        ...
        self.loadshape = inputs.loadshape
        self.incomingT_F = inputs.incomingT_F
        self.supplyT_F = inputs.supplyT_F
        self.recirc_loss = (inputs.supplyT_F - inputs.returnT_F) * inputs.flow_rate * rhoCp * 60. #BTU/HR

class MensDorm(Building):
    def __init__(self, inputs):
        self.magnitude = inputs.magnitude * 18.9 # ASHREA GPD per student
        super().__init__(inputs)

class WomensDorm(Building):
    def __init__(self, inputs):
        self.magnitude = inputs.magnitude * 16.4 # ASHREA GPD per student
        super().__init__(inputs)

class Motel(Building):
    def __init__(self, inputs):
        self.magnitude = inputs.magnitude * 28.8 # ASHREA GPD per unit
        super().__init__(inputs)

class NursingHome(Building):
    def __init__(self, inputs):
        self.magnitude = inputs.magnitude * 20.1 # ASHREA GPD per bed
        super().__init__(inputs)

class OfficeBuilding(Building):
    def __init__(self, inputs):
        self.magnitude = inputs.magnitude * 1.11 # ASHREA GPD per person
        super().__init__(inputs)

class FoodServiceA(Building):
    def __init__(self, inputs):
        self.magnitude = inputs.magnitude * 11.032 # ASHREA GPD per meal
        super().__init__(inputs)

class FoodServiceB(Building):
    def __init__(self, inputs):
        self.magnitude = inputs.magnitude * 6.288 # ASHREA GPD per meal
        super().__init__(inputs)

class Apartment(Building):
    def __init__(self, inputs):
        self.magnitude = inputs.magnitude * 42.8 # ASHREA GPD per unit
        super().__init__(inputs)

class ElementarySchool(Building):
    def __init__(self, inputs):
        self.magnitude = inputs.magnitude * 1.081 # ASHREA GPD per student
        super().__init__(inputs)

class JuniorHigh(Building):
    def __init__(self, inputs):
        self.magnitude = inputs.magnitude * 3.27 # ASHREA GPD per student
        super().__init__(inputs)

class SeniorHigh(Building):
    def __init__(self, inputs):
        self.magnitude = inputs.magnitude * 3.02 # ASHREA GPD per student
        super().__init__(inputs)
    
class MultiFamily(Building):
    def __init__(self, inputs):
        # supplyT_F, incomingT_F, returnT_F, flow_rate, loadshape, n_people, gpdpp, nApt, Wapt, nBR = None
        ...
        with open(os.path.join(os.path.dirname(__file__), '../data/load_shapes/multi_family.json')) as json_file:
            ...
            if isinstance(gpdpp, str): # if the inputs here is a string get the get the gpdpp
                ...
                if gpdpp.lower() == "ca" :
                    if not hasattr(inputs, 'nBR') or inputs.nBR is None or sum(inputs.nBR) == 0:
                    ...
                    for ii in range(0,6):
                        daily_totals += inputs.nBR[ii] * np.array(dataDict['ca_gpdpp'][str(ii) + "br"]) # daily totals is gpdpp * bedroom

                    # Get the 98th percentile day divide by the number of people rounded up to an integer.
                    gpdpp = round(np.percentile(daily_totals,98)/ sum(inputs.nBR), 1)

                # Else look up by normal key function
                else:
                    gpdpp = dataDict['gpdpp'][gpdpp][0] # TODO error handle
            
            self.magnitude = gpdpp * inputs.magnitude # gpdpp * number_of_people

        super().__init__(inputs)
        if(hasattr(inputs, 'nApt') and inputs.nApt > 0 and hasattr(inputs, 'Wapt') and inputs.Wapt > 0):
            self.recirc_loss = inputs.nApt * inputs.Wapt * W_TO_BTUHR
